...
## This script uses Tkinter to create a GUI for option pricing using Binomial Tree and Black-Scholes Models for both
## European and American plain vanilla options. Note that this code contains only graphical elements since core functions are defined separately.

# *The code could seem longer than it should be, however tkinter does not permit to use certain methods together with the 'grid' method that
#  define the position of every widget* #
...
# Start adding some features to it like the title.
root.title("Option Calculator")
root.grid_columnconfigure(0, weight=1) # Block the height of the root
root.grid_rowconfigure(0, weight = 1)  # Block the length of the root

# Create the second-level objects, called frames, to organise better the interface.
# See the pdf report to understand the graphical structure.
...
# Create some labels to define the inputs. Note that the grid method is used to position them within the frame.
...
# Create auxiliary variables associated to check buttons, these allow to assign a boolean value used to check boxes.
...
# Other inputs are created in the form of check buttons, namely for the option type and exercise style.
...
# The above check boxes are positioned within the frame using the 'grid' method.
...
# Define the output variables and their positions.
...
# Define the Option Pricing function that will be actived by the user using a specific button.
def OptionPricing():
    ...
    # Using the function from Options.py, different scenarios are defined.
    ...
    if american_check.get() == TRUE and call_check.get() == TRUE and european_check.get() == FALSE and put_check.get() == FALSE:
        ...
        messagebox.showerror("ERROR", "Check that you have correctly specify all the inputs.") # If something is inserted wrong, the an error message appears.

    # These lines of code are needed to delete the content of the entry boxes.
    ...
    # These lines below insert the values, converted as strings, obtained.
...
# The following function is the one associated to Reset button, it is used to clear everything.
def cancel():
...
# Define outputs: both labels and entries and their positions using grid.
...
# The final step is to position the seven frames defined above.
